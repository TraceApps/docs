...
!!! note "Needs a recent CookTrace"
    This needs CookTrace 1.3.0 or later, which is on CookTrace's dev channel (the `:dev` image) until 1.3.0 is released. CookTrace 1.2.0 and earlier don't have the shopping list API. If linking says CookTrace is too old, update CookTrace.
...
NoteTrace refuses private and loopback addresses by default, so a link can't be used to probe your network. If CookTrace runs on your LAN (`http://192.168.1.20:3003`), on the same host (`http://localhost:3003`), or on the same Docker network (`http://cooktrace:3003`), set this on the NoteTrace server:
