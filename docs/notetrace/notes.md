...
A drawing is an image attachment whose `drawing` column keeps its strokes as JSON: the sheet (`paper` or `dark`, and a grid), and each stroke's tool, colour, size, and points (with stylus pressure). The PNG in `url` is what every screen, export, and search sees; the strokes are only sent when the drawing is opened for editing (`GET /api/notes/:id/attachments/:uuid/drawing`), so a note list stays light, and they sync with the attachment. The server checks a drawing's shape before storing it and caps it at 5,000 strokes and 4 MB. Editing replaces the picture and the strokes together.
