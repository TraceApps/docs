...
Text notes use a rich editor with bold, italic, underline, strikethrough, highlight, headings, lists, checkboxes, quotes, and code. What's saved is plain Markdown, so the same note reads correctly in an export or any other Markdown app. Type **/** for slash commands (headings, lists, a checkbox, an image, a reminder, and more), and press **?** in the notes list for keyboard shortcuts. See [Keyboard shortcuts and slash commands](shortcuts.md).

**Formatting by tap or click.** Select some text and a small bar appears over it (under it on a phone, clear of the system's copy menu) with **Bold**, **Italic**, **Underline**, **Highlight**, **Link**, and **Clear Formatting**. The full set is in the formatting bar above the text on a computer, and behind the **Formatting** button on a phone. **Link** turns the selected words into a link (a bare `example.com` gets `https://`); with the cursor in a link it edits it, and clearing the address removes it. **Clear Formatting** takes bold, italic, highlight, and links off the selection. A computer nests list items and checkboxes with **Tab** and **Shift+Tab**; a phone has **Indent** and **Outdent** buttons in its formatting row.

**Checkboxes in a text note** let you tick things off in the middle of your writing, the way Apple Notes does: tap the checkbox button in a phone's bottom bar or the formatting bar, type `[ ]` and a space at the start of a line, or type `/checkbox`. Enter adds the next one, and a ticked line fades and strikes through. They're saved as Markdown task items (`- [ ]` and `- [x]`) and show as boxes on the note's card. They belong to the note: for items with due dates, repeats, and a place in [Tasks](#tasks), use a checklist.
...
The bar along the bottom holds reminder, Ask Trace, Send to CookTrace, share, add image, attach file, drawing, record voice note, color, labels, and text/checklist, with **More Options** (the three dots) for archive, print, save as template, version history, and trash. Buttons only appear where they apply: Ask Trace on text notes once Trace is set up, Send to CookTrace on checklists once CookTrace is linked. On a phone the bar stays above the keyboard while you type and keeps a short row: **Add** (image, voice note, checkboxes), **Formatting** (swaps the bar to bold, headings, lists, and the rest), color, reminder, and **More Options** for labels, sharing, Trace, CookTrace, archive, version history, and trash.
...
Tap the magnifier in the header (or press `/`) to search. The header turns into a search box that says what you're searching (Search Notes, Search Archive, or a label), and three filters slide in under it: **Type** (Lists, Text, Images, Voice, Files, Reminders, Shared, Links), **Color**, and **Label**. Each opens the same kind of menu (a sheet at the bottom of a phone) where you tick as many as you like; the filter then says what it's set to, like "Lists +1", or shows the colours you picked. Picks within a filter match any of them, and different filters must all match. The funnel button clears every filter, **Clear** in a menu clears just that one, and the ✕ or **Esc** closes search and clears everything. Matching words are marked on the cards, and a note found by a word inside a voice note, an image, or a file shows that line with a microphone, image, or file icon. Your recent searches sit in one row under an empty search box, ready to run again. On a phone, the + button steps aside while you search. The search box searches titles, note bodies, checklist items, voice note transcripts, text read from images, and the text inside PDFs and text files together within the view you're in (Notes, Archive, Trash, or a label), and matches word beginnings as you type, so `jelly` finds "Jellyfin". Press **Ctrl+K** (**Cmd+K** on a Mac) from anywhere to jump to it.
