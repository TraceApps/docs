...
-   ![NoteTrace](assets/img/notetrace-logo.png){ width="120" .app-card-icon }

    **NoteTrace**

    Notes, checklists, and reminders. A self-hosted Google Keep.

    [:octicons-arrow-right-24: NoteTrace docs](notetrace/index.md)
...
Start with **[Get Started → Install with Docker Compose](getting-started/compose.md)**. One page covers every app with per-app tabs.

If you already have one Trace app running and want to add another, jump to **[How the apps compare](self-hosting/compare.md)** for the volume, port, and base-image differences.

## Shared plumbing

Written once, applies to every app:
...
- **[Environment reference](self-hosting/env-vars.md)** lists the env vars the apps share in one comparison table.
